...
class SystemTTSEngine(TTSEngine):
        ...
        try:
            ...
            self._voices = [
                VoiceInfo(
                    id=v.id,
                    name=v.name,
                    language=getattr(v, 'languages', ['unknown'])[0] if hasattr(v, 'languages') else 'unknown',
    ...
    async def synthesize(
        ...
        effective_emotion_mult = 1.0 + (emotion_rate_mult - 1.0) * settings.emotion_intensity
        effective_rate = int(150 * settings.speed * effective_emotion_mult)
        
        logger.debug(f"TTS Rate: base=150, speed={settings.speed}, emotion={settings.emotion.value}, "
